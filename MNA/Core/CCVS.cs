...
namespace Sparky.MNA.Core
{
    ...
    public class CCVS : Component
    {
        ...
        public override void Stamp(CoordinateStorage<double> A, double[] Z, double dt = 0)
        {
            ...
            if (ctrlP != 0) A.At(k1, ctrlP, 1);
            if (ctrlN != 0) A.At(k1, ctrlN, -1);

            // KCL for input current:
            //   A[ctrlP, k1] += 1   (I_in leaves ctrlP)
            //   A[ctrlN, k1] -= 1   (I_in enters ctrlN)

            if (ctrlP != 0) A.At(ctrlP, k1, 1);
            if (ctrlN != 0) A.At(ctrlN, k1, -1);
            ...
            if (outP != 0) A.At(k2, outP, 1);
            if (outN != 0) A.At(k2, outN, -1);
            A.At(k2, k1, -rm);
            ...
            if (outP != 0) A.At(outP, k2, 1);
            if (outN != 0) A.At(outN, k2, -1);
